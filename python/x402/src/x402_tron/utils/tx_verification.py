...
class BaseTransactionVerifier(ABC):
    ...
    async def verify_transaction(
        ...
        self._logger.info(
            f"[EXPECTED] Payment: {expected_from} → {expected_pay_to} | "
            f"{expected_amount} {token_address}"
        )
        if fee_amount > 0 and expected_fee_to:
            self._logger.info(
                f"[EXPECTED] Fee: {expected_from} → {expected_fee_to} | "
                f"{fee_amount} {token_address}"
            )
        else:
        ...
        try:
            ...
            for i, transfer in enumerate(transfers, 1):
                self._logger.info(
                    f"[ACTUAL] #{i}: {transfer.from_addr} → {transfer.to_addr} | "
                    f"{transfer.amount} {transfer.token}"
                )
            ...
            for transfer in transfers:
                ...
                if normalized_to == expected_pay_to:
                    if transfer.amount >= expected_amount:
                        ...
                        self._logger.info(
                            f"[COMPARE] ✓ Payment matched: {transfer.from_addr} → "
                            f"{transfer.to_addr} | {transfer.amount} (expected: {expected_amount})"
                        )
                    else:
                        self._logger.warning(
                            f"[COMPARE] ✗ Payment MISMATCH: got {transfer.amount}, "
                ...
                if expected_fee_to and normalized_to == expected_fee_to:
                    if transfer.amount >= fee_amount:
                        fee_verified = True
                        self._logger.info(
                            f"[COMPARE] ✓ Fee matched: {transfer.from_addr} → "
                            f"{transfer.to_addr} | {transfer.amount} (expected: {fee_amount})"
                        )
                    else:
                        self._logger.warning(
                            f"[COMPARE] ✗ Fee MISMATCH: got {transfer.amount}, "
                            f"expected {fee_amount}"
                        )

            if not payment_verified:
                self._logger.error(
                    f"[COMPARE] ✗ Payment NOT FOUND: expected {expected_from} → "
                    f"{expected_pay_to} | {expected_amount}"
                )
                self._logger.info("=" * 60)
                return TransactionVerificationResult(
                    success=False,
            ...
            if not fee_verified:
                self._logger.error(
                    f"[COMPARE] ✗ Fee NOT FOUND: expected {expected_from} → "
                    f"{expected_fee_to} | {fee_amount}"
                )
                self._logger.info("=" * 60)
                return TransactionVerificationResult(
                    success=False,
