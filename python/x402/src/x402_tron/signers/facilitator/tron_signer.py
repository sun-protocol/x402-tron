...
class TronFacilitatorSigner(FacilitatorSigner):
    ...
    async def verify_typed_data(
        ...
        try:
            ...
            logger.info(
                f"Signature verification: expected_tron={address}, "
                f"expected_evm={expected_evm}, recovered={recovered}"
            )
    ...
    async def write_contract(
        ...
        try:
            ...
            if "BANDWITH_ERROR" in error_msg or "bandwidth" in error_msg.lower():
                logger.error(
                    "BANDWIDTH ERROR: The account does not have enough bandwidth "
